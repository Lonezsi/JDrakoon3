...
// ---------------------------------------------------------------
// Windows input backend. A single long-lived PowerShell process loads a
// tiny Win32 P/Invoke helper once, then we feed it one command per line:
//
//   M <dx> <dy>   relative mouse move        S <amount>   scroll wheel (signed)
//   C / R         left / right click         K <name>     special key
//   D / U         left button down / up      T <base64>   type UTF-8 text
//   X <mods> <k>  key combo (mods held while key taps)
//
// This is the original InputControlService driver, unchanged — relocated
// behind the InputBackend interface so other OSes can plug in alongside.
// ---------------------------------------------------------------
...
export class WindowsInputBackend implements InputBackend {
  enabled = true;
  ...
  scroll(amount: number) {
    this.write(`S ${Math.round(amount)}`);
  }
  tapKey(name: KeyName) {
    this.write(`K ${name}`);
  }
